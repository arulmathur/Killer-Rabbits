...
private:
    ...
    int    m_row;
    int    m_col;
    // You'll probably find that a rabbit object needs additional
    // data members to support your implementation of the behavior affected
    // by poisoned carrots.
 
  //Arul's extra data members
    bool m_dead; //keeps track of if rabbit is alive or dead
    int m_carrotsEaten; //keeps track of number of carrots eaten
    int m_postCarrotMoves; //keeps track of number of moves made after carrots eaten for poison effects
...
Rabbit::Rabbit(Arena* ap, int r, int c)
{
    ...
    m_dead = false; //rabbit is initially alive
    m_carrotsEaten = 0; //initially eaten 0 carrots
    m_postCarrotMoves = 0; //initially made no moves after eating a carrot
}
...
void Rabbit::move()
{
    ...
    if ((m_carrotsEaten == 1 && m_postCarrotMoves % 2 == 0) || m_carrotsEaten == 0) { //if the rabbit has eaten a carrot, move every even turn after eating it; otherwise, move normally if not dead
        int rabbitDirection = randInt(0, 3);
        attemptMove(*m_arena, rabbitDirection, m_row, m_col);
 
        if (m_arena->getCellStatus(m_row, m_col) == HAS_POISON) { //if the post move rabbit square has poison
            m_carrotsEaten++; //increment carrots eaten by this rabbit
            m_arena->setCellStatus(m_row, m_col, EMPTY); //clear the carrot from the arena
            if (m_carrotsEaten == 2) { //if eaten 2 carrots, the rabbit is dead
                m_dead = true;
            }
        }
    }
...
string Player::move(int dir)
{
    ...
    bool couldMove = attemptMove(*m_arena, dir, m_row, m_col);
 
    if (couldMove) {
        if (m_arena->numberOfRabbitsAt(m_row, m_col) != 0) { //if rabbit at position, die
            m_dead = true;
    ...
    return "Player couldn't move; player stands."; //only makes it here if it couldn't move
}
...
{
    int count = 0;
    for (int i = 0; i < rabbitCount(); i++) { //loops through rabbits array and checks how many rabbits are at supplied position
        if (m_rabbits[i]->col() == c && m_rabbits[i]->row() == r) {
            count++;
        }
    }
    return count;
...
void Arena::display(string msg) const
{
    ...
    for (int i = 0; i < rabbitCount(); i++) {
        int row = m_rabbits[i]->row();
        int col = m_rabbits[i]->col();
        char numRabbits = '0' + numberOfRabbitsAt(row, col); //converts integer output by function into char
        if (numRabbits == '1') {
            displayGrid[row - 1][col - 1] = 'R';
        }
        else if (numRabbits != '0' && numRabbits < '9') {
            displayGrid[row - 1][col - 1] = numRabbits;
        }
...
bool Arena::addRabbit(int r, int c)
{
    ...
    if (rabbitCount() == MAXRABBITS) {
        ...
        m_nRabbits++;
        Rabbit* ptr = new Rabbit(this, r, c);
        m_rabbits[rabbitCount()-1] = ptr;
        return true;
    }
}
...
void Arena::moveRabbits()
{
    ...
    for (int i = 0; i < rabbitCount(); i++) {
        m_rabbits[i]->move();
        if (m_rabbits[i]->row() == m_player->row() && m_rabbits[i]->col() == m_player->col()) { //if any rabbit coord matches the player coord, player is dead
            m_player->setDead();
        }
    }
 
    Rabbit* aliveRabbits[MAXRABBITS]; //create new array to keep track of living rabbits
    int numAlive = 0;
    
    for (int i = 0; i < rabbitCount(); i++) {
        if (m_rabbits[i]->isDead()) { //if rabbit is dead, deallocate it
            delete m_rabbits[i];
        }
        else {
            aliveRabbits[numAlive] = m_rabbits[i]; //if its alive, add it to the new array
            numAlive++;
        }
    }
    
    m_nRabbits = numAlive;
    for (int i = 0; i < m_nRabbits; i++) { //rewrite original array with new array of alive rabbits
        m_rabbits[i] = aliveRabbits[i];
    }
...
{
    if (dir == NORTH) {
        if (r - 1 >= 1) { //if the row is stil >= 1, make the move
            r--;
            return true;
        }
    }
    else if (dir == SOUTH) {
        if (r + 1 <= a.rows()) { //if the row is still <= number of rows in arena, make the move
            r++;
            return true;
        }
    }
    else if (dir == EAST) {
        if (c + 1 <= a.cols()) { //if the col is still <= number of cols in arena, make the move
            c++;
            return true;
        }
    }
    else if (dir == WEST) {
        if (c - 1 >= 1) { //if the col is still >= to 1, make the move
            c--;
            return true;
        }
    }
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    int dangerZones[MAXROWS][MAXCOLS]; //create array to store dangerous areas for player to move
    for (int i = 0; i < a.rows(); i++) { //populate danger array with all zero values to begin with
        for (int j = 0; j < a.cols(); j++) {
            dangerZones[i][j] = 0;
        }
    }
    for (int i = 1; i <= MAXROWS; i++) { //iterate through real arena
        for (int j = 1; j <= MAXCOLS; j++) {
            int rabbitCount = a.numberOfRabbitsAt(i, j);
            int dangerRow = i-1;
            int dangerCol = j-1;
            if (rabbitCount != 0) {
                // if possible to move north
                if (i - 1 >= 1) {
                    dangerZones[dangerRow-1][dangerCol]+= rabbitCount;
                }
                // if possible to move south
                if (i + 1 <= a.rows()) {
                    dangerZones[dangerRow+1][dangerCol]+= rabbitCount;
                }
                // if possible to move east
                if (j + 1 <= a.cols()) {
                    dangerZones[dangerRow][dangerCol+1]+= rabbitCount;
                }
    ...
    int direction = -1;
    int leastDanger = dangerZones[r-1][c-1];
    //check all of the possible directions, if it has less danger than a direction already checked, make it the new suggestion
    if (r - 1 >= 1 && a.numberOfRabbitsAt(r-1, c) == 0) {
        if (dangerZones[r - 1 - 1][c - 1] < leastDanger) {
            leastDanger = dangerZones[r - 1 - 1][c - 1];
            direction = NORTH;
        }
    }
    ...
    if (c + 1 <= a.cols() && a.numberOfRabbitsAt(r, c+1) == 0) {
        if (dangerZones[r - 1][c + 1 - 1] < leastDanger) {
            leastDanger = dangerZones[r - 1][c + 1 - 1];
            direction = EAST;
        }
    }
